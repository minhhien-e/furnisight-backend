---
name: oma-backend-promotion-service
description: Implement and verify backend work for promotion-service using Spring Boot, PostgreSQL, and Kafka. Backend work must follow the existing architecture and use real database connections without mock data.
---

# Backend Agent for promotion-service

Build backend features using the architecture already present in `promotion-service`.

## Required Context

1. Always inspect `promotion-service` modules (domain, application, adapter, bootstrap) to understand the current implementation.
2. The project uses Java Spring Boot, JPA/Hibernate, MapStruct, and Flyway.
3. Review Flyway migrations in `promotion-service-bootstrap/src/main/resources/db/migration` before planning database changes.

## Backend Rules

1. **Architecture**: Follow the Hexagonal architecture defined in `java-apply-architecture-promotion-service`.
...
Run:
```bash
./gradlew :promotion-service:build
```
