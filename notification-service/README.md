...
Notification Service áp dụng kiến trúc **Hexagonal Architecture (Ports and Adapters)** và được chia thành 4 module chính:
...
├── notification-domain/           # [CORE] Cốt lõi nghiệp vụ
│   ├── src/main/java/com/furnisight/notification/domain/
│   │   ├── entity/                # Entity nghiệp vụ lõi (vd: Notification)
│   │   ├── exception/             # Lỗi Domain
│   │   └── value/                 # Value Objects
│   └── build.gradle
├── notification-application/      # [USE CASES] Điều phối các thao tác
│   ├── src/main/java/com/furnisight/notification/application/
...
│   └── build.gradle
├── notification-adapter/          # [INFRASTRUCTURE] Giao tiếp với thế giới bên ngoài
│   ├── src/main/java/com/furnisight/notification/adapter/
...
│   └── build.gradle
└── notification-bootstrap/        # [ENTRYPOINT] Nơi khởi chạy Application
    ├── src/main/java/com/furnisight/notification/bootstrap/
    │   └── NotificationApplication.java # Chứa hàm Main
    └── build.gradle
```

### Chi Tiết Các Module

#### 1. `notification-domain` (Core)
...
#### 2. `notification-application` (Use Cases)

**Vai trò**: Chứa Port (in/out) và Use Case Service. Nó chỉ depends vào `notification-domain`.
**Tại sao cần tách riêng?**: Điều phối nghiệp vụ, không phụ thuộc framework bên ngoài.

#### 3. `notification-adapter` (Infrastructure)
...
#### 4. `notification-bootstrap` (Entrypoint)
