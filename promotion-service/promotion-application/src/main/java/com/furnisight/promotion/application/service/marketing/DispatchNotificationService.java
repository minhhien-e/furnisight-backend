...
public class DispatchNotificationService implements DispatchNotificationUseCase {
    ...
    public void dispatchNotification(DispatchNotificationQuery query) {
        List<MarketingChannel> channels = helper.parseChannels(helper.split(query.getNotification().getChannels()));
        List<MarketingNotificationGateway.Recipient> recipients = helper.resolveRecipients(
                query.getNotification().getTargetType().name(), helper.split(query.getNotification().getTargetUserIds()), query.getNotification().getSegmentKey(), channels);
        Map<String, Object> metadata = helper.buildVoucherMetadata(query.getNotification().getRelatedVoucherId());
        MarketingNotificationGateway.DispatchResult result = notificationGateway.send(
                query.getNotification().getTitle(),
                query.getNotification().getBody(),
                "/notifications",
                channels,
                recipients,
                metadata);
        query.getNotification().setSentCount(result.sentCount());
        query.getNotification().setStatus(query.getNotification().getSendType() == MarketingSendType.DRAFT ? CampaignStatus.DRAFT : CampaignStatus.SENT);
        query.getNotification().setDispatchedAt(LocalDateTime.now());
        notificationRepository.save(query.getNotification());
        helper.logBatch("NOTIFICATION", query.getNotification().getId(), query.getNotification().getTitle(), query.getNotification().getBody(), channels, recipients, result);
    }
}
