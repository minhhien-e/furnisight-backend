...
import com.furnisight.promotion.application.port.in.usecase.UpdateComboUseCase;
...
public class UpdateComboService implements UpdateComboUseCase {
    ...
    @Override
    @Transactional
    public MarketingComboDto updateCombo(UpdateComboQuery query) {
        PromotionCombo combo = comboRepository.findById(query.getId())
                .orElseThrow(() -> new IllegalArgumentException("Combo not found"));
        helper.applyCombo(combo, query.getCommand());
        comboRepository.save(combo);
        comboItemRepository.deleteByComboId(query.getId());
        helper.saveComboItems(combo, query.getCommand());
        helper.recalculateCombo(query.getId());
        return helper.toComboDto(comboRepository.findById(query.getId()).orElse(combo));
    }
}
