...
public class Review {
    ...
    public Review(ReviewTitle title, UUID userId, UUID productId, UUID orderItemId,
        ...
        if(userId== null ){
            throw new ReviewDomainException(ErrorCode.USER_ID_MISSING);
        }
        if(productId== null){
            throw new ReviewDomainException(ErrorCode.PRODUCT_ID_MISSING);
        }
        if( orderItemId== null){
            throw new ReviewDomainException(ErrorCode.ORDER_ITEM_ID_MISSING);
        }
        if(title== null || title.value().isBlank()){
            throw new ReviewDomainException(ErrorCode.TITLE_MISSING);
        }
    ...
    public void approve() {
        ...
        if (this.status != ReviewStatus.PENDING) {
            throw new ReviewDomainException(ErrorCode.INVALID_REVIEW_STATE,
    ...
    public void updateTrustScore(BigDecimal newScore) {
        if (this.status == ReviewStatus.ARCHIVED) {
            throw new ReviewDomainException(ErrorCode.REVIEW_ALREADY_ARCHIVED);
        }
    ...
    public boolean updateFromUser(String newTitle, String newContent, Integer newRating) {
       if (!"VISIBLE".equalsIgnoreCase(this.status.name())) {
            throw new ReviewDomainException(ErrorCode.REVIEW_IN_PROCESS);
        }
