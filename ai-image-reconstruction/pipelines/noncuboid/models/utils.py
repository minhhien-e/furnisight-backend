...
from pipelines.noncuboid.models.loss import _gather_feat, _sigmoid, _transpose_and_gather_feat

try:
    import numba
    jit = numba.jit
except Exception:  # pragma: no cover
    # Fallback for inference-only environments where numba is absent.
    def jit(*args, **kwargs):
        def decorator(func):
            return func
        return decorator
...
def DisplayLayout(
    ...
    cv2.imwrite(os.path.join(output_dir, f'{iters}_select.png'),
