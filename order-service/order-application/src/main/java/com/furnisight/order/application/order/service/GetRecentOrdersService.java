package com.furnisight.order.application.order.service;

import com.furnisight.order.application.order.port.in.usecase.GetRecentOrdersUseCase;
import com.furnisight.order.application.order.port.in.query.GetRecentOrdersQuery;
import com.furnisight.order.domain.entities.order.Order;
import com.furnisight.order.domain.repository.order.OrderRepository;
import java.util.List;
import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

@Service
@RequiredArgsConstructor
public class GetRecentOrdersService implements GetRecentOrdersUseCase {

    private final OrderRepository orderRepository;

    @Override
    @Transactional(readOnly = true)
    public List<Order> getRecentOrders(GetRecentOrdersQuery query) {
        int limit = query.getLimit() > 0 ? query.getLimit() : 5;
        return orderRepository.findAll(0, limit);
    }

}
